...
The `home-routes` handler in the `movie-finder.routes.home` namespace
...
The `home-page` function will in turn call the `movie-finder.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `movie-finder.handler` namespace:
...
Request middleware functions are located under the `movie-finder.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `movie-finder.dev-middleware` namespace found in
