...
(defn generic-template-introduction-action [entry]
  (let [sender-id (keyword (str (get-in entry [:sender :id])))]
    (typing-on sender-id)
    (post-messenger sender-id :message {:text "Les templates de type génériques permettent de présenter l'information sous forme de listes verticales"})))

(defn generic-template-description-step-one-action [entry]
  (let [sender-id (keyword (str (get-in entry [:sender :id])))]
    (typing-on sender-id)
    (post-messenger sender-id :message {:text "Ils peuvent s'accompagner de boutons pour amener l'utilisateur à effectuer une action."})))

(defn generic-template-action [entry]
  (let [sender-id (keyword (str (get-in entry [:sender :id])))]
    (typing-on sender-id)
...
(defn generic-template-continue-action [entry]
  (let [sender-id (keyword (str (get-in entry [:sender :id])))]
    (post-messenger sender-id :message {:attachment {:type    "template"
...
(def generic-template-route
  (routes (messenger-route
            ...
            #{:read}
            (fn [entry]
              :generic-template-description-step-one-action)
            generic-template-introduction-action)
          (messenger-route
            :generic-template-description-step-one-action
            #{:read}
            (fn [entry]
              :generic-template-action)
            generic-template-description-step-one-action)
          (messenger-route
            :generic-template-action
            #{:read}
